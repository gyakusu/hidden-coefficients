...
信頼pt・VA提案ゲート・交差項・昇進・上申といった**隠れた仕組み**が、成果を裏で左右します。
それらが何かを**観測から自力で見抜く**のがこのゲームの核心です。

さらに、ゲーム開始時に**架空の部署へ配属**され、**どの活動が主力の変数で、どれが定数か**は
配属（シード）ごとに入れ替わります。「前の部署の常識」は次では通用しません。

> 🔒 **種明かし（ネタバレ）は [`docs/mechanics-spoilers.md`](./docs/mechanics-spoilers.md) に隔離しています。**
> 未プレイの方は開かないでください——一度読むと初回の体験は取り戻せません。
